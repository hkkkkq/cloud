...
/**
 * @author liwh
 * @Title: DataConvertUtils
 * @Package com.lwhtarena.nl.utils.common
 * @Description: 数据类转换
 * @Version 1.0.0
 * @date 2020/5/15 08:51
 */
public class DataConvertUtils {
    ...
    public static DateConvert getDateConvert(Annotation[] annotations) {
        ...
        if (null != annotations && annotations.length > 0) {
            List<Annotation> annotationList = Arrays.asList(annotations).stream()
                    .filter(anno -> anno instanceof DateConvert).collect(Collectors.toList());
